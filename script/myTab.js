...
/*
es5버전의 객체지향입니다
장점 : 호환성이 좋다, es6 이전 버전은 모두 적용이 가능함
단점 : 프로토타입으로 메서드를 정의하기 때문에 es6클래스 문법에 비해 복잡합니다
그리고 ()=>{} 화살표함수를 사용하지 않기 때문에 함수 내부에서 this를 고정시키는
...
function MyTab(el){
    ...
    // 여기서 this는 인스턴스를 의미
    console.log(this);
    //여기서의 this는 인스턴스를 의미하는것
}

/*
prototype은 자바스크립트에서 객체 생성자 함수의 프로퍼티입니다.
객체 생성자 함수를 정의할 때, prototype를 사용해서 해당 생성자 함수로
생성된 모든 인스턴스가 공유할 수 있는 메서드나 속성을 정의합니다
이렇게 생성자 함수의 prototype에 메서드, 속성을 추가하면
생성된, 생성될 모든 인스턴스가 해당 메서드, 속성을 복사해서 가져가는 것이 아닌
공유하면서 사용할 수 있기 때문에 메모리 절약과 효율성을 높일 수 있습니다
*/
...
MyTab.prototype.bindingEvent = function(){
    this.btns.forEach(function(el, index){
        ...
        // 이벤트 밖에서 내부 this객체 값을 인스턴스 this로 고정을
        // 하도록 bind로 묶어줌
    }.bind(this))
    ...
    MyTab 인스턴스 객체를 참조해야 동작할 수 있기 때문에
    bind로 고정합니다 그 결과 this.active를 참조할 때도 올바른 문맥의
    MyTab을 this로 유지할 수 있습니다
    */
}
...
bind(값)은 bind앞의 값의 this를 값으로 고정시킴
*/
