...
콜백함수로 예를 들 수 있는데, 함수를 다른 함수의 인자로 전달할 수 있고,
실행 시 동적으로 함수를 생성해서 전달도 가능합니다
btns.addEventListener("click",()=>{})

2. 모든 요소는 함수의 반환 값이 될 수 있습니다
함수는 다른 함수의 반환 값으로도 사용될 수 있습니다
function aaa(){
    return function bbb(){}
}

3. 모든 요소는 할당 명령문의 대상이 될 수 있습니다
함수는 변수에 할당될 수 있습니다
...
// highFunction은 매개변수로 콜백함수를 가진다는 뜻이며, 콜백함수를 반환한다는 뜻
...
불변성 - 변경할 수 없는 상태, 혹은 그런 값을 의미합니다
즉, 한번 생성되면 그 값을 변경할 수 없는 것
이는 데이터의 변경을 허용하지 않고, 새로운 데이터를 생성하거나 
반환하는 방식으로 동작해야 한다는 의미입니다
...
// 기존값
let tu2 = tu1 + (4);
// 새로운 값을 만들 때 기존값을 복사해서 새로운 값을 추가하고, 
// 결과 새로운 데이터가 생성됨
print(tu1)
print(tu2)
...
두가지로 나눌 수 있습니다. 익명함수표현식과 명명함수표현식
const add1 = function(){} << 익명함수표현식
const add2 = function bbb(){} << 명명함수표현식

특징은 변수에 담기 전까지는 생성되지 않는다고 보기 때문에 호이스팅에서
자유롭습니다
...
{자신을 this로 바인딩하지 않는다. constructor로 쓰일 수 없다
prototype을 가지고 있지 않는다. yield키워드를 허용하지 않아서 gnenrator로 
쓸 수 없다. this가 의미가 달라진다}

4. 생성자 함수(constructor function)
function person(name) {this.name = name};
// 이런 종류의 객체지향코드나, 클래스문법이 존재해야 합니다
const person = new person("다혜");
new키워드를 사용해서 객체 인스턴스를 생성하는 함수

5. 메서드(method)
메서드는 내부안에 존재하는 명령의 형태에 함수를 지칭합니다
...
6. 함수 생성자(Function consturctor)
Funtction생성자를 사용해서 함수를 동적으로 생성하는 방법입니다
함수 구문안에서 문자열로 함수를 정의하는 방법이지만, 특수상황에서만
사용하는 것을 권장하고 주의해야합니다
...
함수를 괄호로 둘러싸고 함수 끝에 추가괄호를 사용해서 함수를 즉시 호출합니다

이점
1. 스코프 격리 : 즉시 실행 함수 내에서 선언된 변수들은
무조건 함수내부에서만 유효하기 때문에 외부의 다른 스코프와 충돌할수가 없다

2. 비공개 변수 : 외부에서 접근할 수 없습니다

3. 초기화 코드 : (단, 코드가 길고, 복잡할 때 사용가능)
초기화 코드를 실행해서 모듈 패턴과 조합하여 모듈화된 코드구현가능

4. 클로저
클로저란 외부 변수에 접근할 수 있는 함수를 의미합니다
함수와 해당함수가 접근할 수 있는 자유변수간의 조합입니다
즉, 외부변수에 대한 참조를 유지하는 내부 함수를 클로저라고 합니다
*/
