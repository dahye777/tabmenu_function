...
메서드를 화살표 함수로 정의하므로 this가 알아서 바인딩되므로 추가작업이 필요없습니다
또한 상속과 같은 객체지향 고급 기능을 추가적으로 구현하기 용이합니다
*/
/*
자바스크립트를 기반으로 하는 프레임워크인 뷰, 리엑트에서는 class를 사용할 수 없습니다
왜? 명령어 class로 들어있기 때문에 사용할 수 없다
react의 경우 js파일에 html이 들어있습니다 그래서 html에 주는 class -> className
*/

class MyTab{
    constructor(){
        // 생성자 메서드 객체 인스턴스를 초기화하는 역할을 합니다
        // 매번 만들어지는 인스턴스를 각 개체에 맞춰서 새로 생성해주는 것
...
class ExTab extends MyTab{
    constructor(){
        super();
        /*
        super();을 호출함으로써 ExTab이라는 클래스는 MyTab클래스의 생성자를 실행합니다
       ...
       // 상속을 통해서 기본적인 작업을 진햏ㅇ하고 추가적인 작업을 실행할 수 있습니다
    }

    addMethod(){
        this.tab.style.backgroundColor = "pink";
        console.log("메서드를 추가했습니다");
    }
}

// 인스턴스를 생성
const extendTab = new ExTab();
extendTab.addMethod();
...
// 다형성
// 다형성은 상속개념을 깔고, 그 위에 동일한 메서드를 각가 다른 클래스의 인스턴스에서
// 다르게 동작하도록 하는 개념
